...
The dashboard includes a Verify & Publish panel for each token. The button runs
server-side BaseScan API checks with `BASESCAN_API_KEY`, attempts the supported
verification path, and validates logo + metadata readiness from inside the app.
